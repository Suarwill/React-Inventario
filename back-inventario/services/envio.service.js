...
const cargarMermas = async (mermas, token) => {
    ...
    const nro = `${fecha.getFullYear().toString().slice(-2)}${String(fecha.getMonth() + 1).padStart(2, '0')}${String(fecha.getDate()).padStart(2, '0')}${String(fecha.getHours()).padStart(2, '0')}${String(fecha.getMinutes()).padStart(2, '0')}`; // Formato 2 cifras año, mes, día, hora, minuto
    ...
    try {
        ...
        for (const merma of mermas) {
            ...
            const query = `
                INSERT INTO movimientos (nro, fecha, origen, destino, tipo, cantidad, codigo)
